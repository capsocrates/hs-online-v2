	...
	fclose($fp);
			...
			$conn = db_connectQuery();
					...
					if (mysql_num_rows($result)<1) {//Add gamename to table
						$query = "insert into hs_rolls(gamename, username, skulls, shields, blanks, ipaddress, gametime) values
					...
					$query2 = "insert into hs_rolls_itemized(gamename, username, action, skulls, blanks, totals, gametime, ipaddress) values
								('".$gameName."', '".$userName."', '1', '".$skull."', '".$blank."', '".$Roll."', NOW(), '".$userIP."')";
